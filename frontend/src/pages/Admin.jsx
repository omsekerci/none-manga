...
function Admin() {
  ...
  const fetchChapters = async (seriesId) => {
    try {
      const res = await axios.get(`http://localhost:5000/api/series/${seriesId}`)
  ...
  const handleSeriesSubmit = async () => {
    try {
      await axios.post('http://localhost:5000/api/series', {
        ...seriesForm,
  ...
  const handleChapterSubmit = async () => {
    try {
      await axios.post('http://localhost:5000/api/chapters', chapterForm)
  ...
  const handlePageSubmit = async () => {
    ...
    try {
      await axios.post(`http://localhost:5000/api/chapters/${pageForm.chapterId}/page`, { url: pageForm.url })
